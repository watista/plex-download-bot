...
class Plex:
    ...
    async def get_media_url(self, media_data: dict, media_type: str) -> Optional[str]:
        ...
        try:
            ...
            await self.log.logger(f"❌ *No valid media found in Plęx for '{title}'* ❌\n\nCheck the error log for more information.", False, "error")
        except Exception as e:
            await self.log.logger(f"❌ *Error while creating a Plęx link* ❌\n\nCheck the error log for more information.", False, "error")
